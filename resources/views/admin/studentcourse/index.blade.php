            ...
            <script type="text/html" id="options">
                <div class="layui-btn-group">
                    @can('student.studentcourse.create')
                        ...
                        <a class="layui-btn layui-btn-warm layui-btn-sm" lay-event="cancel">取消</a>
                        {{--                            @endif--}}
                        {{--                        @endforeach--}}
            ...
            layui.use(['layer', 'table', 'form'], function () {
                ...
                var dataTable = table.render({
                    ...
                    , cols: [[ //表头
                        ...
                        , {field: 'status',width: 150, title: '是否已选这门课'}
                ...
                table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'add') {
                        ...
                        layer.confirm('确认选择吗？', function (index) {
                            $.get('/admin/studentcourse/' + data.id + '/cancel', {_token: "{{csrf_token()}}"}, function (result) {
                                if (result.code == 0) {
                                    //     obj.del(); //删除对应行（tr）的DOM结构
                                    //     obj.reload();
                                    obj.update({
                ...
                $("#listDelete").click(function () {
                    ...
                    if (ids.length > 0) {
                        layer.confirm('确认删除吗？', function (index) {
                            $.post("{{ route('admin.studentcourse.destroy') }}", {
                                _method: 'delete',
                                ids: ids
                            }, function (result) {
                                if (result.code == 0) {
